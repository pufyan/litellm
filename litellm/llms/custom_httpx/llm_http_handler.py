...
class BaseLLMHTTPHandler:
    ...
    async def _open_realtime_backend_ws(
        ...
        *,
        ...
        """Open the backend realtime websocket via the shared connector.

        Thin seam over :class:`RealtimeBackendConnector` so provider handlers
        and tests share one handshake-retry implementation; the connector also
        travels with the session for reconnects.
        """
        from litellm.litellm_core_utils.realtime_backend_connector import RealtimeBackendConnector

        connector: Final = RealtimeBackendConnector(
            url=url,
            headers=headers,
        ...
        return await connector.connect(websockets_module)
    ...
    async def async_realtime(
        ...
        try:
            ...
            backend_connector: Final = RealtimeBackendConnector(
                url=url,
                headers=headers,
                ssl_context=ssl_context,
                open_timeout=8.0,
